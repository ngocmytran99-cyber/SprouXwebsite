...
const CreatorsGrid: React.FC<CreatorsGridProps> = ({ content }) => {
  ...
  const cardBtnLabel = content?.['creators-view-profile-btn'] || 'View Profile';
  const footerMsg = content?.['creators-footer-msg'] || 'Join us in reshaping the future of creator-led knowledge products—and unlock privileges worth up to <span class="text-primary font-bold">$1,000</span> per creator.';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div key={i} className="group bg-white rounded-3xl p-6 border border-slate-200 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden relative">
               ...
               <div className="aspect-[4/5] rounded-2xl overflow-hidden mb-6 relative">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                     <button className="w-full py-2 bg-white/20 backdrop-blur-md text-white rounded-lg border border-white/30 text-xs font-bold uppercase tracking-widest">{cardBtnLabel}</button>
                  </div>
               </div>
        ...
        <div className="text-center">
           <div className="inline-flex flex-col items-center gap-6">
              <button className="px-10 py-5 bg-primary text-white font-bold rounded-2xl text-lg flex items-center gap-2 hover:bg-teal-800 transition-all shadow-xl shadow-teal-700/20 group">
                {sectionCta}
                <ArrowUpRight size={22} className="group-hover:-translate-y-1 group-hover:translate-x-1 transition-transform" />
              </button>
              <p 
                className="text-slate-500 font-medium max-w-lg mx-auto leading-relaxed"
                dangerouslySetInnerHTML={{ __html: footerMsg }}
              />
           </div>
        </div>
      </div>
