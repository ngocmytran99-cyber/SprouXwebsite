...
const PostList: React.FC<PostListProps> = ({ posts, setPosts, categories, user, onEditPost, onNewPost }) => {
  ...
  const filteredPosts = useMemo(() => {
    return posts.filter(post => {
      ...
      const matchesCategory = postCategoryFilter === 'all' || post.categoryIds.includes(postCategoryFilter);
  ...
  const executeBulkDelete = () => {
    if (bulkActionToConfirm) {
      setPosts(posts.filter(p => !bulkActionToConfirm.ids.includes(p.id)));
      setSelectedPostIds([]);
  ...
  const executeTrash = () => {
    if (itemToTrash) {
      setPosts(posts.filter(p => p.id !== itemToTrash));
      setItemToTrash(null);
    }
  };
              ...
              <tr key={post.id} className="hover:bg-[#f6f7f7] group transition-colors">
                ...
                <td className="px-3 py-4 align-top">
                   <div className="text-[#2271b1] flex flex-wrap gap-1">
                      {post.categoryIds.length > 0 ? post.categoryIds.map((cid, idx) => (
                        <span key={cid} className="hover:underline cursor-pointer">
                          {categories.find(c => c.id === cid)?.name}{idx < post.categoryIds.length - 1 ? ',' : ''}
                        </span>
                      )) : '—'}
                   </div>
                </td>
