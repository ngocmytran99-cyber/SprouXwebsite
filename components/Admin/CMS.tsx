...
import { User, BlogPost, Category, MediaAttachment, PageData, ContentBlock, HelpDeskCategory, HelpDeskTopic, HelpDeskArticle } from '../../types';
...
const CMS: React.FC<CMSProps> = ({ 
  ...
  users, setUsers,
  headerLogo, setHeaderLogo,
  footerLogo, setFooterLogo,
  // Lifted props
  ...
  const handleSavePost = async (data: Partial<BlogPost>, shouldClose: boolean = false): Promise<void> => {
    ...
    // Kiểm tra slug trùng (loại trừ bài đang sửa)
    ...
    // Chuẩn bị dữ liệu bài viết hoàn chỉnh (Upsert)
    ...
    setPosts(prevPosts => {
      ...
      if (exists) {
        ...
        return [postData, ...prevPosts]; // Bài mới lên đầu
      }
    });

    setHasUnsavedChanges(false);
    
    // Logic điều hướng sau khi lưu
    if (shouldClose) {
